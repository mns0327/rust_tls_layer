...
impl fmt::Display for TlsHandshakeProtocol {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.handshake_type == Handshake_type::ClientHello {
            write!(f, "  - Handshake type: {}\n  - Version: {}\n  - Random: {}\n  - Session ID: {}\n  - Cipher Suites: {}", self.handshake_type.to_string(), self.version, self.random.to_vec().hex_display(), self.session_id.to_vec().hex_display(), self.cipher_suites.as_ref().unwrap().iter().map(|s| s.to_string()).collect::<Vec<String>>().join(", "))
        } else {
            write!(f, "  - Handshake type: {}\n  - Version: {}\n  - Random: {}\n  - Session ID: {}\n  - Cipher Suites: {}", self.handshake_type.to_string(), self.version, self.random.to_vec().hex_display(), self.session_id.to_vec().hex_display(), self.chosen_cipher.as_ref().unwrap().to_string())
        }
    }
}
