...
impl HandshakeFragment {
    ...
    pub fn from_vec(vec: Vec<u8>, msg_type: HandshakeType) -> Self {
        let fragment = match msg_type {
            HandshakeType::client_hello => HandshakeFragment::ClientHello(ClientHello::from_vec(vec)),
            HandshakeType::server_hello => HandshakeFragment::ServerHello(ServerHello::from_vec(vec)),
...
impl ServerHello {
    ...
    fn from_vec(vec: Vec<u8>) -> Self {
        let random = vec[0..32].try_into().unwrap();
        let session_id_len = vec[32];
        let session_id = vec[33..33 + session_id_len as usize].try_into().unwrap();
        let cipher_start = 33 + session_id_len as usize;
        ...
        Self { random, session_id, chosen_cipher, compression_method }
    }
}
...
impl Handshake {
    pub fn new(handshakeType: u32) -> Self {
        ...
        let fragment: HandshakeFragment = match msg_type {
            HandshakeType::client_hello => {
                HandshakeFragment::ClientHello(ClientHello::new())
            }
            HandshakeType::server_hello => {
                HandshakeFragment::ServerHello(ServerHello::new())
            }
            _ => {
                panic!("Unsupported handshake type: {:?}", msg_type);
            }
        };
        Self { msg_type, version: ProtocolVersion::new(3, 3), length: 0, fragment: fragment }
    }
    ...
    pub fn to_vec(&mut self) -> Vec<u8> {
        ...
        self.length = self.fragment.to_vec().len() as u32 + 2;
        let length_vec = u32::to_be_bytes(self.length);
        vec.extend([length_vec[1], length_vec[2], length_vec[3]]);
        vec.extend([self.version.major, self.version.minor]);
    ...
    pub fn from_vec(vec: Vec<u8>) -> Self {
        ...
        let fragment = HandshakeFragment::from_vec(vec[6..].to_vec(), msg_type);
        Self { msg_type, version: ProtocolVersion::new(vec[4], vec[5]), length, fragment }
    }
}