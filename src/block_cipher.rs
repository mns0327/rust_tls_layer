pub fn ECB_MODE(encrypt_func: fn([u8; 16], [u8; 16]) -> [u8; 16], data: Vec<u8>, key: Vec<u8>, iv: Vec<u8>) -> Vec<u8> {
    let mut result = Vec::new();
    let padded_data = pad(data, 16);
    for i in (0..padded_data.len()).step_by(16) {
        result.extend(encrypt_func(padded_data[i..i+16].try_into().unwrap(), key.clone().try_into().unwrap()));
    }
    result
}
